...
//    else {wrReg8(0x160, rdReg8(0x160)&~0x12);}
    ...
    wrReg16(0x184, _param.previewCropHStart); wrReg16(0x188, _param.previewCropVStart);
    if (div < 1) div = 1;
    if (_param.previewCropHSize <= 320) { // 160x120, 320x240
        _param.previewHRatio=div-1; _param.previewVRatio=div-1;
    } else
    if (_param.previewCropHSize <= 640) { // 640x480
        _param.previewHRatio=div; _param.previewVRatio=div;
    } else
    if (_param.previewCropHSize <= 1280) { // 800x600, 1024x762, 1280x1024
        _param.previewHRatio=div+1; _param.previewVRatio=div+1;
    } else {
        _param.previewHRatio=div+2; _param.previewVRatio=div+2;
    }
    ...
    if (_param.previewCropHSize <= 320) { // 160x120, 320x240
        wrReg16(0x18C, 320/2*div); wrReg16(0x190, 240/2*div);
    } else
    if (_param.previewCropHSize <= 640) { // 640x480
        wrReg16(0x18C, 640/2*div); wrReg16(0x190, 480/2*div);
    } else
    if (_param.previewCropHSize <= 1280) { // 800x600, 1024x762, 1280x1024
        wrReg16(0x18C, 1280/2*div); wrReg16(0x190, 960/2*div);
    } else {
        wrReg16(0x18C, 2560/2*div); wrReg16(0x190, 1920/2*div);
    }

    uint32_t addr=((uint32_t)x_pos+(uint32_t)y_pos*320)>>1;
    wrReg32(0x19C,addr); wrReg32(0x1A0,addr); wrReg32(0x1B0,addr); wrReg32(0x1B4,addr);
    if(on) {wrReg8(0x160, rdReg8(0x160)&~(0x09)); wrReg8(0x160, rdReg8(0x160)|0x12);}
    else {wrReg8(0x160, rdReg8(0x160)&~0x12);}
