...
uint16_t  SDC::StreamRGBFrame(FSFILE *fileStream, uint16_t numSectors) {
    ...
    while (sectorsToRead < numSectors) {
        if (FileGetNextCluster( fileStream, 1) != CE_GOOD) {
// Add!!!
Serial.println('A');
            return 0xFFFF;
        }
...
uint8_t  SDC::JPEGWriteToSD(uint32_t fsize, uint16_t index) {
    FSFILE *pFile=NULL;
    uint8_t  jpeg_encode_byte;
    uint8_t  file_ext[5]=".jpg"; uint8_t file_name[9]="J";
    uint8_t  file_index[3]; uint32_t i=0, len;
    uint32_t en_jpeg_address=rdReg32(0x414)<<2;
    sprintf(file_index, "%d", index); strcat(file_name, file_index); strcat(file_name, file_ext);
    if (FSInit())  {
