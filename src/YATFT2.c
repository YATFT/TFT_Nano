...
void  YATFT::begin(uint16_t id, int8_t shift, uint8_t mirr_h, uint8_t mirr_v) {
...
void  YATFT::LCD_SetCursorPosition(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
...
void  YATFT::PutPixel(int16_t x, int16_t y) {
    int32_t address; address = FRAME_SIZE*_page + (_line_mem_pitch<<1)*y + (x<<1);
